...
preditores_mod3 <- c(
  ...
  'raio_planetario_terra', 'temp_equilibrio_k', 
  'num_planetas_sistema', 'gravidade_estrela_log', 'raio_estrela_sol'
)
...
planetas <- planetas_raw %>%
  ...
  drop_na(any_of(preditores_mod3)) %>%
...
f_modelo3 <- as.formula(paste("koi_disposition ~", paste(preditores_mod3, collapse = " + ")))

set.seed(42)
modelo_bayesiano_3 <- stan_glm(
  formula = f_modelo3,
  data = dados_treino,
...
mcmc_trace(
  modelo_bayesiano_3, 
  pars = preditores_mod3
) + ggplot2::ggtitle("Trace Plots de Preditores Chave (Modelo Reduzido)")
...
matriz_amostras <- as.matrix(modelo_bayesiano_3)[, preditores_mod3]

tabela_hpd <- hdi(matriz_amostras, credMass = 0.95)

modelo_bayesiano_3 %>%
  gather_draws(!!!syms(preditores_mod3)) %>%
...
print(summary(modelo_bayesiano_3, digits = 3))

preditores_mod2_intercept <- c(preditores_mod3, "(Intercept)")
...
# Teste
matriz_prob_treino <- posterior_epred(modelo_bayesiano_3)
matriz_prob_teste <- posterior_epred(modelo_bayesiano_3, newdata = dados_teste)
