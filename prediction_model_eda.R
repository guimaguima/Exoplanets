...
preditores_mod <- c(
  'log_periodo_orbital_dias_z', 
  'log_parametro_impacto_z', 
  'log_duracao_transito_hrs_z', 
  'log_raio_planetario_terra_z', 
  'temp_estrela_k_z', 
  'log_raio_estrela_sol_z',
  
  'log_raio_planetario_terra_z:log_parametro_impacto_z',
  'log_periodo_orbital_dias_z:log_parametro_impacto_z',
  'log_periodo_orbital_dias_z:log_duracao_transito_hrs_z'
)
...
planetas <- planetas_raw %>%
  ...
  select(koi_disposition, periodo_orbital_dias, duracao_transito_hrs, 
         raio_planetario_terra, parametro_impacto, temp_estrela_k, raio_estrela_sol) %>%
  drop_na() %>%
  mutate(

    koi_disposition = factor(koi_disposition, levels = c("FALSE POSITIVE", "CONFIRMED")),
    
    log_periodo_orbital_dias = log10(periodo_orbital_dias + 1),
    log_parametro_impacto = log10(parametro_impacto + 1),
    log_duracao_transito_hrs = log10(duracao_transito_hrs + 1),
    log_raio_planetario_terra = log10(raio_planetario_terra + 1),
    log_raio_estrela_sol = log10(raio_estrela_sol + 1)
  ) %>%
  mutate(across(
    c(log_periodo_orbital_dias, log_parametro_impacto, log_duracao_transito_hrs, 
      log_raio_planetario_terra, temp_estrela_k, log_raio_estrela_sol), 
